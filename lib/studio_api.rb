...
module SLMS
  module Studio
	...
	"#{prefix(prefix_options)}#{collection_name}#{query_string(query_options)}"
      ...
      def self.upload ( path, base_system )
        begin
          file = ::File.new( path, "r" )
          #upload the rpm
          r = PostRequest.new "rpms?base_system=#{base_system}"
        ...
        raise StudioRequestException.new("Failed to upload rpm: '#{xml.inspect}'") unless success
      end
    end
     
    class RpmFile
      def self.copy_to_disk (hsh={} )
        file_id = hsh[:id]
        file_name = hsh[:name]
        
        applianceId = hsh[:applianceId]
        data, success = GetRequest.request "/rpms/#{file_id}/data"
